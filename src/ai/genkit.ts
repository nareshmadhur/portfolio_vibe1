...
if (googleApiKey) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.warn(
      'GOOGLE_API_KEY is not set in .env.local. AI features using Google AI will not work.'
    );
  } else {
    // In production, you might want to throw an error or ensure this is handled.
    console.warn(
      'GOOGLE_API_KEY is not set. AI features using Google AI will be unavailable in production.'
    );
  }
}
